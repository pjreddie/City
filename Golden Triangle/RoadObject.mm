...
- (void) calculateRoadPolygons {
	...
	// Lane Seperator
	[wallPolygons addObject:[[BoundingPolygon alloc] initWithCoord:[self generateRectangleFromLine:CONST_LANE_SEPERATOR_SIZE x1:intersectionx1 y1:y1+0.01 z1:intersectionz1 x2:intersectionx2 y2:y2+0.01 z2:intersectionz2] andColorRed:1.0 green:0.0 blue:1.0]];
	// sidewalk
	[wallPolygons addObject:[[BoundingPolygon alloc] initWithCoord:[self generateRectangleFromLine:totalRoadWidth x1:intersectionx1 y1:y1-0.01 z1:intersectionz1 x2:intersectionx2 y2:y2-0.01 z2:intersectionz2] andColorRed:1.0 green:1.0 blue:1.0]];
...
- (NSArray *) generateRectangleFromLine:(double)width x1:(double)x_1 y1:(double)y_1 z1:(double)z_1 x2:(double)x_2 y2:(double)y_2 z2:(double)z_2{
	...
	deltaX = (width/2)*cos(angle);
	deltaZ = (width/2)*sin(angle);
